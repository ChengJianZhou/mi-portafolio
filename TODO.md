...
- [x] Set up domain (marcoszhou.dev comprado)
- [x] Dockerize frontend con multi-stage build (Node → serve)
- [x] Nginx reverse proxy en servidor local
- [x] Deploy en red local via marcoszhou.es
- [ ] Alias `deploy` en servidor para ejecutar deploy.sh desde cualquier carpeta
- [ ] **Auto-deploy con GitHub webhook** — que el servidor haga pull+build automáticamente en cada `git push`, sin conectarse por SSH
